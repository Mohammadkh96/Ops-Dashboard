...
export function SyncStatus() {
  ...
  const refresh = useMutation({
    mutationFn: () => apiFetch<Refresh>("/paymaxis/refresh", { method: "POST" }),
    onSuccess: (data) => {
      setFailed(false);
      setLastOkAt(data.lastOkAt);
      setConfigured(data.configured);
      setUpstreamError(data.error);
      // Only when something was actually pulled: refetching every view on a
      // rate-limited no-op would reload the whole dashboard for nothing.
      if (data.ran) void queryClient.invalidateQueries();
    },
  ...
  useEffect(() => {
    ...
    const ask = () => {
      if (document.visibilityState === "visible") refresh.mutate();
    };
    ask();
    ...
    <button
      ...
      title={
        !configured
          ...
          : (upstreamError ?? "Pull new payments from Paymaxis now")
      }
      className={cn(
        "flex items-center gap-1.5 rounded-lg border border-border bg-card px-2.5 py-1.5 text-[11px] font-medium transition-colors hover:text-foreground",
        tone,
      )}
    >
      <RefreshCw
