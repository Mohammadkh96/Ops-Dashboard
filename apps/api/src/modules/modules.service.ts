...
/**
 * Serves the operational module datasets. Representative data is returned so
 * the API is populated in a fresh environment; where the database has data it
 * is layered in (e.g. gateway rows). Shapes mirror the frontend's types.
 */
@Injectable()
export class ModulesService {
  constructor(private readonly prisma: PrismaService) {}

  private readonly clients = ['#48213', '#10042', '#55210', '#22981', '#77120', '#30918', '#66203', '#12844', '#90117', '#40551'];
  private readonly countries = ['AE', 'DE', 'GB', 'FR', 'SA', 'NG', 'IN', 'SG', 'BR', 'ES'];
  private readonly gatewayNames = ['ForumPay', 'LimePay', 'Paystrax', 'Coinbase', 'Stripe', 'Nuvei', 'Bridge'];
  private readonly methods = ['Card', 'Crypto', 'Bank', 'Local'];
  private readonly ccy = ['USD', 'EUR', 'GBP', 'AED', 'BTC'];
  private readonly txStatuses = ['approved', 'processing', 'pending', 'review', 'declined', 'failed', 'refunded'];
  private readonly risks = ['low', 'low', 'low', 'medium', 'medium', 'high', 'critical'];
  ...
  async gateways() {
    ...
    try {
      const rows = await this.prisma.paymentGateway.findMany({ orderBy: { name: 'asc' } });
  ...
  incidents() {
    return [
      { id: 'INC-104', title: 'ForumPay gateway offline', severity: 'critical', status: 'investigating', owner: 'Yusuf Ali', impact: 'All ForumPay deposits failing; ~$180K/hr volume affected', rootCause: 'Upstream PSP API returning 503 since 14:02 UTC', openedAt: '18m ago', timeline: [{ time: '14:02', text: 'Automated alert: success rate dropped to 0%' }, { time: '14:05', text: 'On-call paged; incident opened at SEV-1' }, { time: '14:11', text: 'PSP status page confirms upstream outage' }, { time: '14:16', text: 'Deposits rerouted to LimePay for affected regions' }] },
      { id: 'INC-103', title: 'Elevated decline rate — Visa EU', severity: 'high', status: 'open', owner: 'Sara Ahmed', impact: 'Card decline rate +14% for EU BINs over 30 min', openedAt: '42m ago', timeline: [{ time: '13:38', text: 'Decline-spike alert triggered for Visa EU' }, { time: '13:44', text: 'Investigating acquirer-side 3DS friction' }] },
      { id: 'INC-102', title: 'Webhook delivery delay — Coinbase', severity: 'medium', status: 'investigating', owner: 'David Chen', impact: 'Deposit confirmations delayed ~40s; no funds at risk', openedAt: '1h ago', timeline: [{ time: '13:20', text: 'Webhook latency breached 10s SLA' }] },
      { id: 'INC-101', title: 'Duplicate transaction anomaly', severity: 'low', status: 'resolved', owner: 'Sara Ahmed', impact: '3 duplicate authorizations auto-voided', rootCause: 'Client double-submit; idempotency key added', openedAt: '4h ago', timeline: [{ time: '10:02', text: 'Anomaly detector flagged duplicates' }, { time: '10:31', text: 'Duplicates voided; fix shipped' }, { time: '10:40', text: 'Resolved' }] },
    ];
  }
  ...
  async users() {
    ...
    try {
      ...
      const titleRole = (r: string) =>
        r.split('_').map((p) => p[0] + p.slice(1).toLowerCase()).join(' ');
      return rows.map((u) => ({
        id: u.id,
